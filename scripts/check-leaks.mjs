...
for (const f of files) {
  ...
  readFileSync(f, 'utf8').split('\n').forEach((line, i) => {
    ...
    const brit = new RegExp(
      ...
      // British currency and idiom on a US site — spelling checks miss these
      'pounds\\s+(worth|sterling)|quid|petrol|autumn|pavement|rubbish|' +
      'postcode|post\\s?code|tarmac|skirting|lorry|torch(es)?\\s+(for|to)' +
      ')\\b', 'i');
